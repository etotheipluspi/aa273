...
%     mu1 = mu1s_up(3:6,i+1);
%     Sigma1 = Sigma1s_up(3:6,3:6,i+1)+;
%     p1 = mvnpdf(y(:,i+1),mu1s_up(:,i+1),Sigma1s_up(:,:,i+1));
%     p2 = mvnpdf(y(:,i+1),mu2s_up(:,i+1),Sigma2s_up(:,:,i+1));
    p1 = mvnpdf(y(:,i+1),C1st*mu1s_pr(:,i+1),C1st*Sigma1s_pr(:,:,i+1)*C1st'+R);
    p2 = mvnpdf(y(:,i+1),C2st*mu2s_pr(:,i+1),C2st*Sigma2s_pr(:,:,i+1)*C2st'+R);
    if p1 > p2
        mtest(1,i+1) = 1;
    else
        mtest(2,i+1) = 1;
...
for i = 1:ns
    ...
    plot(t,xs(i,:)); hold on
    plot(t,mu1s_up(i,:),'r')
    title(['Estimating state ' num2str(i), ', EKF 1'],'fontsize',14)
end
...
for i = 1:ns
    ...
    plot(t,y(i,:)); hold on
    plot(t,y1s_up(i,:),'r')
    title(['Estimating measurement ' num2str(i), ', EKF 1'],'fontsize',14)
end
...
for i = 1:ns
    ...
    plot(t,xs(i,:)); hold on
    plot(t,mu2s_up(i,:),'r')
    title(['Estimating state ' num2str(i), ', EKF 2'],'fontsize',14)
end
...
for i = 1:ns
    ...
    plot(t,y(i,:)); hold on
    plot(t,y2s_up(i,:),'r')
    title(['Estimating measurement ' num2str(i), ', EKF 2'],'fontsize',14)
end
